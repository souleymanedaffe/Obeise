...
with st.form("input_form"):
    ...
    fcvc   = st.slider("FCVC (fréquence de légumes/jour)", 0.0, 5.0, step=0.1, value=2.0)
    ...
    faf    = st.slider("FAF (heures d’activité physique/semaine)", 0.0, 5.0, step=0.1, value=1.0)
    tue    = st.slider("TUE (heures passées sur écrans/jour)", 0.0, 5.0, step=0.1, value=2.0)
...
if submitted:
    ...
    # Encodage des catégories avec les mêmes LabelEncoders
    for col, le in le_dict.items():
        if col in raw.columns:
            raw[col] = le.transform(raw[col])
    
    # Préparation de X_new
    ...
    st.success(f" **Classe prédite : {class_map[pred]}**")
